...
const HOVER_SEL =
  'a[href], button, [data-cursor], [data-sound], [data-project-slug], [role="button"], input, textarea, select, summary, label';
...
function hoverRoot(node) {
  if (!(node instanceof Element)) return null;
  if (node.closest("[data-sound='off']")) return null;
  return node.closest(HOVER_SEL);
}

function bindUiCues() {
  if (uiCuesBound || typeof window === "undefined") return;
  uiCuesBound = true;
  const coarse =
...
/** Play a hover tick when the pointer enters a new interactive object. */
export function notePointerHover(node) {
  const root = hoverRoot(node);
  if (root === lastHoverEl) return;
  lastHoverEl = root;
  if (root) playHover();
}
...
/** Soft tick when the cursor enters a clickable object. */
export function playHover() {
  ...
  osc.type = "square";
  osc.frequency.setValueAtTime(1100 + Math.random() * 220, t);
...
/** Rolling tick while dragging the globe (or any spin control). */
export function playDragTick(intensity = 0.5) {
  if (reducedMotion || typeof window === "undefined") return;
  const now = performance.now();
  if (now - lastDragAt < 40) return;
  lastDragAt = now;
  ...
  withAudio(() => {
    ...
    osc.type = "square";
    osc.frequency.setValueAtTime(150 + amt * 280 + Math.random() * 40, t);

    filter.type = "lowpass";
    filter.frequency.setValueAtTime(380 + amt * 420, t);
    filter.Q.setValueAtTime(0.7, t);

    const peak = 0.012 + amt * 0.018;
    gain.gain.setValueAtTime(0.0001, t);
