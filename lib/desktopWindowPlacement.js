...
export function getDeterministicDesktopPositions({
  ...
  const junkLeft = Math.max(
    edge,
    Math.round(skillsLeft - W.junk - pad)
  );
  let junkTop = Math.round(skillsTop + H.skills + pad);
  const maxJunkTop = projTop - pad - H.junk;
  if (junkTop > maxJunkTop) {
    junkTop = Math.max(topBand, maxJunkTop);
  }
  if (junkLeft + W.junk + pad > skillsLeft) {
    pos.junk = {
      left: Math.max(edge, skillsLeft),
      top: Math.round(skillsTop + H.skills + pad),
    };
  } else {
    pos.junk = { left: junkLeft, top: junkTop };
  }
