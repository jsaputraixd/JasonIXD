...
export function getDeterministicDesktopPositions({
  ...
  const statusBarClearance = Math.round(68 + 12 * layoutScale);
  const iconTop = Math.max(topBand, Math.round(vh - iconH - statusBarClearance));
  ...
  pos.trashIcon = {
    left: Math.round(vw - iconW - edge - RIGHT_RESERVE),
    top: iconTop,
  };
