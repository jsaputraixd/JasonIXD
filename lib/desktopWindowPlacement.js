...
export const CONTACT_BANNER_FRAME_INSET = 10;
...
export function getDeterministicDesktopPositions({
  ...
  const packRight = Number.isFinite(gridMaxRight) ? gridMaxRight : vw;
  const gridTop = Math.round(
    vh - BOTTOM_RESERVE - gridH - Math.round(PROJECT_GRID_LIFT * layoutScale)
  );
  let gridLeft = Math.round((vw - gridW) / 2);
  gridLeft = Math.max(
    edge,
    Math.min(gridLeft, Math.max(edge, packRight - gridW))
  );
  ...
  const bannerW = Math.round(
    Math.max(168, Math.min(200, 184 * layoutScale))
  );
  const canvasRight = vw - CONTACT_BANNER_FRAME_INSET;
  const bannerGap = canvasRight - (welcomeLeft + W.welcome);
  const bannerLeft = Math.round(
    welcomeLeft + W.welcome + (bannerGap - bannerW) / 2
  );
  pos.contactBanner = {
    left: bannerLeft,
    width: bannerW,
  ...
  const folderPairW = folderIconW * 2 + folderIconGap;
  const folderPad = Math.round(14 * layoutScale);
  const folderRowTop = Math.max(edge, gridTop - folderIconH - folderPad);

  let folderRowLeft = Math.round(gridLeft + (gridW - folderPairW) / 2);
  folderRowLeft = Math.max(
    edge,
    Math.min(folderRowLeft, vw - edge - folderPairW)
  );
  ...
  pos.otherProjectsIcon = {
    left: folderRowLeft + folderIconW + folderIconGap,
    top: folderRowTop,
    width: folderIconW,
    height: folderIconH,
  };
