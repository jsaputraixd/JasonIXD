...
export const PROJECT_HERO_COLS = 2;
...
function fitThumbBox(aspect, layoutScale, { hero = false } = {}) {
  const s = layoutScale;
  const maxW = Math.round((hero ? 400 : 252) * s);
  const maxH = Math.round((hero ? 248 : 300) * s);
  const minW = Math.max(108, Math.round((hero ? 200 : 118) * s));
...
export function fitProjectCardsToBounds(
  ...
  const cols = cards.length <= 2 ? Math.max(1, cards.length) : PROJECT_GRID_COLS;
...
export function getProjectDesktopCards(projectList, layoutScale) {
  const hero = (projectList?.length ?? 0) <= 2;
