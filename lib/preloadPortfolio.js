...
const ASCII_PORTRAIT = "/images/Website Avatar.png";
...
export function preloadPortfolioAssets() {
  ...
  const first = desktopFeaturedProjects[0];
  ...
  scheduleIdle(() => {
    desktopFeaturedProjects.slice(1).forEach((p) => {
  ...
  // Prefetch hero case studies only — archiving every /work route at boot
  // was competing with the live desktop for bandwidth and decode time.
  scheduleIdle(() => {
    desktopFeaturedProjects.forEach((p) => {
      if (p?.slug) preloadRoute(`/work/${p.slug}`);
    });
  });
}
