...
### Particle background (built in code, not a PNG)
Before the parallax layers, render an abstract space-like background using pure CSS + JS:
- Deep dark gradient: `background: radial-gradient(ellipse at 50% 30%, #1a1a2e 0%, #171717 70%)`
- 60–80 floating particles using `<canvas>` or absolutely positioned `<div>` elements
- Each particle: 1–3px circle, either white (`opacity: 0.3–0.6`) or teal (`#2DD4BF`, `opacity: 0.2–0.4`)
- Each particle drifts slowly upward at a random speed (0.2–0.8px per frame), looping back to bottom when it exits top
- Subtle, atmospheric — not distracting. Think floating dust in a dark room, not a screensaver.
- Use `requestAnimationFrame` for smooth animation
- Disable on mobile for performance (`@media (pointer: coarse)` — show static gradient only)

### PNG illustration style
All desk layer PNGs should follow the **bubbly inflated 3D** aesthetic:
- Objects look slightly puffed up with soft rounded edges
- Smooth gradient fills on all surfaces, no harsh edges
- Subtle inner glow and soft drop shadows
- Think: clay render, inflated UI icons, soft Blender aesthetic
- This is illustrative not photorealistic
- Generated via Midjourney using the bubbly claymation prompts (see midjourney-desk-prompts.md)
