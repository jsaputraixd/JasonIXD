...
- Teal ring cursor — default system cursor kept visible. Teal ring (36px, no fill) appears on hover around the cursor. Expands to 56px with label on project/enter elements. No cursor: none.
...
- Croissant is the center element — small and quiet (40px, no spin). Feels like a wink, not a repeat of the hero. Tags are the star of this section.
...
## Risk Decisions (Locked May 15)
| Risk | Decision |
|------|----------|
| Croissant density | Gate only gets full theatrical treatment. Skills gets small quiet cameo (40px, no spin). Wink not punchline. |
| Custom cursor | Softened — default cursor kept, teal ring on hover only. No cursor: none. Preserves accessibility. |
| Skills orbit | Phase 1 — build it. If it slips schedule, ship spine without it and add in v1.1. |
| Time-to-work | Sketchbook must be reachable within 2 scrolls. Hero placeholder must never block content. |
| Metaphor stack | Spine ships first. Delight layers (orbit, full parallax, videos) are additive not load-bearing. |
